 ...
 *
 *  Created on: 2019-10-05T20:40:13.486Z
 *      Author: ALex
 */
...
int jsonGetTelnet(RailTelnet_st *railTelnet,RailJson_st* railJson){
	TRACE(printf("\tget JSON Telnet %p\n",railTelnet););
	char *tables[32];
	uint16_t iTables = 0;
	tables[0] = NULL;
	printf("\t%s %s %s\n",railTelnet->argv[1],railTelnet->argv[2],railTelnet->argv[3]);
  	...
  	fflush(stdout);
		switch(((Hitch_st *)car)->command){
			...
			case ROUTE_JSON_GET_TELNET:
			{
				...
				printf("railTelnet %p\r\n",railTelnet);
  			fflush(stdout);
				jsonGetTelnet(railTelnet,&railJson_0);
